...
|-----------|---------------|-------------|-----------|
| Factor risk attribution | Multi-factor model (CAPM, FF3, Carhart) | Single-factor cross-check | `run_factor_model` + `risk_metrics` |
| Portfolio optimisation | Black-Litterman with views | Mean-variance optimisation | `run_black_litterman` + `risk_adjusted_returns` |
| Risk-parity allocation | ERC / inverse-vol weighting | Factor-based cross-check | `calculate_risk_parity` + `run_factor_model` |
| Stress testing | Historical + hypothetical scenarios | VaR/CVaR comparison | `run_stress_test` + `risk_metrics` |
| Portfolio optimization | Mean-variance efficient frontier | Black-Litterman with views | `optimize_mean_variance` + `optimize_black_litterman_portfolio` |
| Risk budgeting / tail risk | Factor risk decomposition | VaR/CVaR tail analysis | `analyze_factor_risk_budget` + `analyze_tail_risk` |
| Trade execution analysis | Spread decomposition + quality | Optimal execution strategy | `analyze_spreads` + `optimize_execution` |
| Statistical arbitrage | Pairs cointegration + backtest | Momentum factor cross-check | `analyze_pairs_trading` + `analyze_momentum` |
| Behavioral bias assessment | Prospect theory analysis | Market sentiment indicators | `analyze_prospect_theory` + `analyze_sentiment` |
| Performance attribution | Brinson-Fachler + factor-based | Tracking error decomposition | `brinson_attribution` + `factor_attribution` |
| Credit portfolio analytics | Gaussian copula credit VaR | Migration mark-to-market | `calculate_portfolio_credit_risk` + `calculate_migration` |
| Macro economic analysis | Taylor rule + Phillips curve + Okun | Recession risk composite | `analyze_monetary_policy` |
| FX macro (PPP/IRP) | Purchasing power parity | Interest rate parity cross-check | `analyze_international` |
| Balance of payments | Current account sustainability | Twin deficit detection | `analyze_international` |
| Credit scoring / PD estimation | Scorecard + Merton structural model | Intensity model + validation | `calculate_scorecard` + `calculate_merton` + `calculate_scoring_validation` |
| Capital allocation / RAROC | Economic capital + risk-adjusted return | Euler/Shapley allocation | `calculate_economic_capital` + `calculate_raroc` + `calculate_euler_allocation` |
| Index construction / smart beta | Weighting + rebalancing + reconstitution | Tracking error + smart beta | `calculate_weighting` + `calculate_rebalancing` + `calculate_tracking_error` + `calculate_smart_beta` |

## Analysis Workflows

### Quantitative Risk Workflow

1. **Factor attribution**: call `run_factor_model` with return series and factor data
...
2. **Black-Litterman optimisation**: call `run_black_litterman` with market data and investor views
...
3. **Risk parity allocation**: call `calculate_risk_parity` with covariance matrix
...
4. **Stress testing**: call `run_stress_test` with portfolio and scenario parameters
...
1. **Build efficient frontier**: call `optimize_mean_variance` with return estimates and covariance
...
2. **Incorporate views via Black-Litterman**: call `optimize_black_litterman_portfolio` with market equilibrium and views
...
1. **Decompose factor risk**: call `analyze_factor_risk_budget` with portfolio and factor data
...
2. **Measure tail risk**: call `analyze_tail_risk` with portfolio positions and return data
...
1. **Analyse market quality**: call `analyze_spreads` with trade and quote data
...
2. **Plan optimal execution**: call `optimize_execution` with order details and market parameters
...
1. **Pairs trading**: call `analyze_pairs_trading` with two correlated asset price series
...
2. **Momentum factor**: call `analyze_momentum` with cross-section of asset returns
...
1. **Prospect theory analysis**: call `analyze_prospect_theory` with investment outcomes
...
2. **Market sentiment**: call `analyze_sentiment` with current market indicators
...
1. **Credit portfolio VaR**: call `calculate_portfolio_credit_risk` with portfolio positions and default parameters
...
2. **Rating migration analysis**: call `calculate_migration` with transition matrix and portfolio
...
1. **Taylor Rule**: call `analyze_monetary_policy` with inflation, output gap, and neutral rate assumptions
...
2. **Phillips Curve**: call `analyze_monetary_policy` with unemployment and inflation data
...
3. **Okun's Law**: call `analyze_monetary_policy` with unemployment and potential output data
...
4. **Recession risk scoring**: call `analyze_monetary_policy` with composite indicator data
...
5. **Purchasing Power Parity**: call `analyze_international` with price levels and exchange rates
...
6. **Interest Rate Parity**: call `analyze_international` with spot rate and interest rates
...
7. **Balance of Payments**: call `analyze_international` with external account data
...
1. **Build scorecard**: call `calculate_scorecard` with applicant data and target variable
...
2. **Merton structural model**: call `calculate_merton` with equity and debt parameters
...
3. **Intensity model**: call `calculate_intensity_model` with CDS spreads and recovery assumption
...
4. **PD calibration**: call `calculate_scoring_validation` with raw PDs and macro factor
...
5. **Model validation**: call `calculate_scoring_validation` with predicted probabilities and outcomes
...
1. **Economic capital**: call `calculate_economic_capital` with portfolio exposures and default parameters
...
2. **RAROC**: call `calculate_raroc` with revenue, costs, and capital data
...
3. **Euler allocation**: call `calculate_euler_allocation` with business unit risks and correlations
...
4. **Shapley allocation**: call `calculate_shapley_allocation` with business units and risk function
...
5. **Limit management**: call `evaluate_limits` with limits and current exposures
...
1. **Choose weighting scheme**: call `calculate_weighting` with constituents and method
...
2. **Manage rebalancing**: call `calculate_rebalancing` with current and target weights
...
3. **Measure replication quality**: call `calculate_tracking_error` with portfolio and benchmark data
...
4. **Construct smart beta**: call `calculate_smart_beta` with factor data and tilt parameters
...
5. **Manage reconstitution**: call `calculate_reconstitution` with universe and eligibility criteria
