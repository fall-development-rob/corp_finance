...
requires_tools:
  - analyze_factor_risk_budget
  - analyze_international
  - analyze_momentum
  - analyze_monetary_policy
  - analyze_pairs_trading
  - analyze_prospect_theory
  - analyze_sentiment
  - analyze_spreads
  - analyze_tail_risk
  - brinson_attribution
  - calculate_economic_capital
  - calculate_euler_allocation
  ...
  - calculate_shapley_allocation
  - calculate_smart_beta
...
|----------|---------|------------|
| `run_factor_model` | Multi-factor regression (CAPM, FF3, Carhart4, Custom) | asset_returns, factor_returns (MKT, SMB, HML, MOM), model_type |
| `run_black_litterman` | Black-Litterman portfolio optimisation with views | market_cap_weights, covariance_matrix, views (absolute/relative), risk_aversion, tau |
| `calculate_risk_parity` | Risk parity portfolio construction | assets, covariance_matrix, method (InverseVol/ERC/MinVariance), target_volatility |
| `run_stress_test` | Multi-scenario stress testing with 5 built-in historical | portfolio positions, scenarios (or use built-in GFC/COVID/etc.), correlation_adjustments |

### Portfolio Optimization

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `optimize_mean_variance` | Markowitz mean-variance: efficient frontier, tangency (max Sharpe) portfolio, global minimum variance, constrained optimal weights, diversification ratio, HHI | asset_names, expected_returns, covariance_matrix, risk_free_rate, constraints (long_only, min/max_weights, sector_constraints), frontier_points, target_return, target_risk |
| `optimize_black_litterman_portfolio` | Black-Litterman portfolio: implied equilibrium returns, posterior estimation with views (absolute/relative), optimal tilted weights, view contribution, tracking error, information ratio | asset_names, market_cap_weights, covariance_matrix, risk_free_rate, risk_aversion, tau, views (Absolute/Relative), view_confidences |

### Risk Budgeting

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `analyze_factor_risk_budget` | Factor-based risk budgeting: per-factor risk contribution, factor exposure, marginal risk, systematic vs idiosyncratic breakdown, concentration analysis, rebalancing to target budgets | asset_names, weights, factor_names, factor_loadings (N x K), factor_covariance (K x K), specific_variances, risk_budgets, rebalance |
| `analyze_tail_risk` | Tail risk: parametric/Cornish-Fisher/historical VaR and CVaR, marginal and component risk, stress testing, maximum drawdown, tail dependence | asset_names, weights, expected_returns, covariance_matrix, confidence_level, time_horizon, distribution (Normal/CornishFisher/Historical), historical_returns, portfolio_value, stress_scenarios |

### Market Microstructure

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `analyze_spreads` | Bid-ask spread decomposition: quoted/effective/realized spreads, adverse selection (Kyle lambda), inventory risk, information share, market quality score, price impact | security_name, trade_data (timestamp, price, volume, side), quote_data (timestamp, bid/ask price/size), analysis_method (Quoted/Effective/Realized/RollModel/KyleModel), daily_volume, market_cap |
| `optimize_execution` | Optimal execution: Almgren-Chriss framework, TWAP/VWAP/IS/POV strategies, market impact/timing risk/opportunity cost estimation, optimal trajectory, adaptive scheduling | security_name, order_size, side, execution_strategy, market_params (price, volume, volatility, spread, impact coefficients), time_horizon, num_slices, urgency, constraints |

### Quantitative Strategies

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `analyze_pairs_trading` | Statistical pairs trading: cointegration, z-scores, half-life, backtested trades, Sharpe ratio | asset_a_name, asset_b_name, asset_a_prices, asset_b_prices, lookback_period, entry_z_score, exit_z_score, stop_loss_z_score, capital |
| `analyze_momentum` | Momentum factor scoring: risk-adjusted rankings, portfolio construction, backtest, crash risk | assets (name, monthly_returns), lookback_months, skip_months, rebalance_frequency, top_n, risk_free_rate |

### Behavioral Finance

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `analyze_prospect_theory` | Prospect theory: loss aversion, probability weighting, certainty equivalent, disposition effect, framing | outcomes (value, probability), reference_point, loss_aversion_lambda, alpha, beta_param, gamma, delta_param |
| `analyze_sentiment` | Market sentiment: fear/greed index, put-call ratio, VIX analysis, fund flows, contrarian signals | market_name, vix_current, vix_sma_50, put_call_ratio, advance_decline_ratio, margin_debt_change_pct, fund_flows, short_interest_ratio |
...
| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `calculate_portfolio_credit_risk` | Portfolio credit risk (Gaussian copula VaR, HHI, granularity adjustment) | exposures (name, exposure, pd, lgd, rating), correlation, confidence_level, num_simulations |
| `calculate_migration` | Rating migration analysis (transition matrix, multi-year default) | transition_matrix, initial_ratings, horizon_years, spreads_by_rating, exposures |

### Macro Economics

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `analyze_monetary_policy` | Monetary policy analysis (Taylor Rule, Phillips Curve, Okun's Law) | current_fed_funds_rate, inflation_rate, target_inflation, gdp_growth, potential_gdp_growth, unemployment_rate, natural_unemployment_rate, output_gap |
| `analyze_international` | International economics (PPP, interest rate parity, BoP, REER) | domestic_country, foreign_country, spot_fx_rate, domestic_inflation, foreign_inflation, domestic_interest_rate, foreign_interest_rate, forward_fx_rate, trade_balance, capital_flows |

### Credit Scoring

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `calculate_scorecard` | Logistic regression scorecard: WoE binning, IV calculation, scorecard points, Gini coefficient, KS statistic | applicant_data, target_variable, features, base_score, pdo (points to double odds), binning_method |
| `calculate_merton` | Merton structural model: asset value/volatility estimation, distance to default, PD, KMV EDF | equity_value, equity_volatility, debt_face_value, risk_free_rate, time_horizon, dividend_yield |
| `calculate_intensity_model` | Reduced-form intensity model: hazard rates from CDS spreads, survival probability, term structure | cds_spreads, tenors, recovery_rate, discount_curve, interpolation_method |
| `calculate_scoring_validation` | PIT/TTC PD calibration + credit model validation: Vasicek single-factor model, Basel IRB correlation, AUC-ROC, accuracy ratio, Gini, Brier score, Hosmer-Lemeshow | raw_pds, rating_grades, macro_factor, calibration_method (PIT/TTC), central_tendency, asset_correlation, predicted_probabilities, actual_outcomes, score_bins, validation_method |

### Capital Allocation

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `calculate_economic_capital` | Economic capital: VaR/ES-based capital, IRB capital requirement (Basel), stress capital buffer, adequacy ratio | exposures, pds, lgds, correlations, confidence_level, capital_method (VaR/ES/IRB), available_capital |
| `calculate_raroc` | RAROC: risk-adjusted return on capital, RORAC, EVA, SVA, spread to hurdle, risk-adjusted pricing | revenue, costs, expected_loss, economic_capital, cost_of_capital, hurdle_rate, tax_rate |
| `calculate_euler_allocation` | Euler risk contribution: marginal capital allocation, diversification benefit, HHI concentration | business_units, standalone_risks, portfolio_risk, correlation_matrix, risk_measure (VaR/ES) |
| `calculate_shapley_allocation` | Shapley value capital allocation: game-theoretic fair allocation (exact N<=8, sampled N>8) | business_units, risk_function, num_samples (for N>8), confidence_interval |
| `evaluate_limits` | Risk limit management: notional/VaR/concentration limits, utilization tracking, breach detection | business_units, limits (notional/VaR/concentration), current_exposures, escalation_thresholds |

### Index Construction

| MCP Tool | Purpose | Key Inputs |
|----------|---------|------------|
| `calculate_weighting` | Index weighting: market-cap, equal, fundamental, free-float with cap constraints, HHI concentration, sector breakdown | constituents, weighting_method (MarketCap/Equal/Fundamental/FreeFloat), cap_constraint, sector_caps |
| `calculate_rebalancing` | Index rebalancing: drift analysis, optimal trade list, transaction cost estimation, turnover metrics, liquidity-adjusted scheduling | current_weights, target_weights, market_data, cost_model, rebalance_frequency, liquidity_data |
| `calculate_tracking_error` | Tracking error analysis: ex-post TE from returns, ex-ante TE from weights/covariance, active share, information ratio decomposition | portfolio_returns, benchmark_returns, portfolio_weights, benchmark_weights, covariance_matrix |
| `calculate_smart_beta` | Smart beta construction: multi-factor tilted weights (value, momentum, quality, low-vol, dividend) with factor exposure and risk analysis | constituents, factor_data, factor_weights, tilt_strength, constraints, risk_model |
| `calculate_reconstitution` | Index reconstitution: member eligibility screening, additions/deletions, buffer zone management, turnover estimation, impact analysis | universe, eligibility_criteria, current_members, buffer_pct, effective_date, market_data |
...
1. `run_factor_model` — multi-factor regression analysis
...
2. `run_black_litterman` — portfolio optimisation with investor views
...
3. `calculate_risk_parity` — risk-based portfolio construction
   - InverseVolatility, EqualRiskContribution (ERC), MinVariance
   - Returns: weights, risk contributions, diversification ratio
4. `run_stress_test` — multi-scenario portfolio stress testing
...
1. `optimize_mean_variance` — Markowitz efficient frontier construction
...
2. `optimize_black_litterman_portfolio` — portfolio optimization with investor views
...
1. `analyze_factor_risk_budget` — factor-based risk decomposition and budgeting
...
2. `analyze_tail_risk` — tail risk measurement and stress testing
...
1. `analyze_spreads` — bid-ask spread decomposition and market quality
...
2. `optimize_execution` — trade execution optimization
...
1. `analyze_pairs_trading` — statistical arbitrage pairs analysis
...
2. `analyze_momentum` — cross-sectional momentum factor
...
1. `analyze_prospect_theory` — Kahneman-Tversky prospect theory analysis
...
2. `analyze_sentiment` — Fear & Greed composite index
...
1. `calculate_portfolio_credit_risk` — portfolio-level credit risk analytics
...
2. `calculate_migration` — rating transition analysis
...
1. `analyze_monetary_policy` — monetary policy framework analysis
...
2. `analyze_international` — open economy analytics
...
1. `calculate_scorecard` — build logistic regression scorecard
...
2. `calculate_merton` — structural model probability of default
...
3. `calculate_intensity_model` — reduced-form hazard rate model
...
4. `calculate_scoring_validation` — calibrate PIT/TTC probabilities
...
5. `calculate_scoring_validation` — validate credit model performance
...
1. `calculate_economic_capital` — compute risk-based capital requirements
...
2. `calculate_raroc` — risk-adjusted pricing and performance
...
3. `calculate_euler_allocation` — proportional risk contribution
...
4. `calculate_shapley_allocation` — game-theoretic fair allocation
...
5. `evaluate_limits` — risk limit monitoring
...
1. `calculate_weighting` — choose and apply weighting scheme
...
2. `calculate_rebalancing` — manage periodic rebalancing
...
3. `calculate_tracking_error` — measure index replication quality
   ...
   - Information ratio = active return / tracking error
4. `calculate_smart_beta` — construct factor-tilted indices
...
5. `calculate_reconstitution` — manage index membership changes
