...
| M&A target | DCF + precedent transactions | LBO floor price | `dcf_model` + `calculate_returns` |
| Leveraged buyout | LBO model with debt service | Sensitivity on exit | `build_lbo` + `build_sensitivity_grid` |
| Merger / acquisition | Accretion/dilution analysis | Breakeven synergy | `analyze_merger` + `credit_metrics` |
...
| PE deal screening | IRR/MOIC analysis | Sensitivity on exit | `calculate_returns` + `sources_and_uses` + `build_sensitivity_grid` |
| GP/LP distribution | Waterfall modelling | Fund fee analysis | `calculate_waterfall` + `calculate_fund_fees` |
| Portfolio review | Risk-adjusted returns | Drawdown analysis | `risk_adjusted_returns` + `risk_metrics` + `kelly_sizing` |
| Fund evaluation | Net IRR after fees | Fee drag analysis | `calculate_fund_fees` + `build_sensitivity_grid` |
| Cross-border tax | WHT analysis + treaty optimisation | Blocker cost-benefit | `calculate_wht` + `calculate_portfolio_wht` + `screen_ubti_eci` |
| GAAP/IFRS comparison | Accounting reconciliation | Materiality assessment | `reconcile_accounting` + `credit_metrics` |
| Fund NAV | Multi-class NAV with equalisation | Fee drag by class | `calculate_nav` + `calculate_fund_fees` |
| GP economics | Revenue decomposition + break-even | Per-professional economics | `calculate_gp_economics` + `build_sensitivity_grid` |
| Investor due diligence | Gross-to-net return analysis | Fee drag vs peers | `calculate_investor_net_returns` + `calculate_fund_fees` |
| Financial modelling | Three-statement model (IS/BS/CF) | Ratio cross-check | `build_three_statement` + `credit_metrics` |
| Monte Carlo valuation | Stochastic DCF simulation | Deterministic DCF base case | `run_mc_dcf` + `dcf_model` |
| Monte Carlo (generic) | Parametric simulation | Scenario analysis | `run_monte_carlo` + `build_sensitivity_grid` |
| Earnings quality screening | Beneish M-Score + Piotroski F-Score | Accrual/revenue quality | `calculate_beneish_mscore` + `calculate_piotroski_fscore` + `calculate_earnings_quality_composite` |
| Dividend valuation | H-Model or multi-stage DDM | Payout sustainability | `calculate_h_model_ddm` + `calculate_multistage_ddm` + `analyze_payout_sustainability` |
| Buyback vs dividend decision | Buyback accretion analysis | TSR attribution | `analyze_buyback` + `calculate_total_shareholder_return` |
| Financial forensics | Benford's Law + DuPont + Z-scores | Peer benchmarking + red flags | `analyze_benfords_law` + `calculate_dupont` + `calculate_zscore_models` + `calculate_red_flag_scoring` |
...
6. **Sensitivity analysis**: call `build_sensitivity_grid` varying WACC and terminal growth
...
1. **Structure the deal**: call `sources_and_uses` for financing table
   - Sources = Uses must balance (equity + debt = EV + fees)
2. **Build debt schedule**: call `build_debt_schedule` for each tranche
3. **Project returns**: call `calculate_returns` with entry/exit equity and interim cash flows
4. **Sensitivity**: call `build_sensitivity_grid` on exit multiple vs EBITDA at exit
5. **Credit check**: call `credit_metrics` at entry leverage to verify serviceability

### LBO Analysis Workflow

1. **Run full LBO**: call `build_lbo` with entry EV, EBITDA, debt tranches, growth assumptions, exit parameters
...
3. **Sensitivity analysis**: call `build_sensitivity_grid` varying exit multiple vs EBITDA growth
...
1. **Run accretion/dilution**: call `analyze_merger` with acquirer/target financials, offer price, consideration type
...
5. **Sensitivity**: call `build_sensitivity_grid` varying synergies vs offer premium

### Waterfall & Fund Economics Workflow

1. **Model GP/LP splits**: call `calculate_waterfall` with total proceeds, invested capital, and tier structure
   - Standard tiers: Return of Capital -> 8% Preferred Return -> GP Catch-Up -> 80/20 Carry Split
2. **Full fund economics**: call `calculate_fund_fees` with fund size, fee rates, hurdle, waterfall type
...
4. **Stress test**: call `build_sensitivity_grid` across bear/base/bull

### Cross-Border Tax Optimisation Workflow

1. **Assess WHT exposure**: call `calculate_wht` for each income stream
...
2. **Portfolio-level analysis**: call `calculate_portfolio_wht` for blended WHT rate
   - Returns per-holding breakdown + optimisation suggestions
   - Flags high-WHT jurisdictions (Swiss 35%, US 30% statutory)
3. **UBTI/ECI screening**: call `screen_ubti_eci` for tax-exempt investors
...
1. **Run reconciliation**: call `reconcile_accounting` with source/target standard
...
1. **Calculate NAV**: call `calculate_nav` with per-class inputs
...
1. **Model GP economics**: call `calculate_gp_economics`
...
2. **Investor gross-to-net**: call `calculate_investor_net_returns`
   - Deduct: management fees, carry, fund expenses, WHT, blocker costs, org costs
   - Fee drag in bps — >300bps is notable
3. **Compare fee structures**: call `build_sensitivity_grid` varying fee rates

### Three-Statement Modelling Workflow

1. **Build integrated model**: call `build_three_statement` with revenue, cost, balance sheet, and debt assumptions
...
5. **Sensitivity**: call `build_sensitivity_grid` varying revenue growth vs margin assumptions
...
2. **Run generic simulation**: call `run_monte_carlo` with variables, distributions, iterations
   - Returns mean, median, std dev, percentiles (5th/25th/50th/75th/95th), min/max
   - Probability of exceeding threshold values
3. **Run Monte Carlo DCF**: call `run_mc_dcf` for stochastic valuation
...
1. **Run Beneish M-Score**: call `calculate_beneish_mscore` with two periods of financial data
...
2. **Run Piotroski F-Score**: call `calculate_piotroski_fscore` for fundamental strength
...
3. **Assess accrual quality**: call `calculate_accrual_quality` for earnings persistence
...
4. **Assess revenue quality**: call `calculate_revenue_quality` for recognition risks
...
5. **Composite score**: call `calculate_earnings_quality_composite` for overall assessment
...
1. **H-Model valuation**: call `calculate_h_model_ddm` for declining growth assumptions
...
2. **Multi-stage DDM**: call `calculate_multistage_ddm` for explicit growth periods
   - N stages with distinct growth rates + terminal Gordon Growth value
   - Use when growth transitions are not linear
3. **Buyback analysis**: call `analyze_buyback` to compare alternatives
...
4. **Payout sustainability**: call `analyze_payout_sustainability` to assess dividend safety
...
5. **Total shareholder return**: call `calculate_total_shareholder_return` for attribution
...
1. **Benford's Law test**: call `analyze_benfords_law` first for data integrity assessment
...
2. **DuPont decomposition**: call `calculate_dupont` to decompose ROE drivers
...
3. **Z-Score distress models**: call `calculate_zscore_models` for comprehensive distress scoring
...
4. **Peer benchmarking**: call `calculate_peer_benchmarking` for relative positioning
...
5. **Red flag scoring**: call `calculate_red_flag_scoring` for composite risk assessment
