#!/usr/bin/env node
/**
 * Phase 29 Wave 12b — Generate zod schemas from exported JSON Schemas (attest domain).
 *
 * Reads every *.json from the cargo test output directory and emits a
 * corresponding .ts file into src/schemas/generated/attest/.
 *
 * Usage (via npm run schemas:gen:attest):
 *   node scripts/gen-attest-schemas.mjs
 */
...
const SCHEMA_SRC = join(WORKSPACE_ROOT, "crates", "corp-finance-core", "target", "schemas", "attest");
const SCHEMA_OUT = join(__dirname, "..", "src", "schemas", "generated", "attest");
