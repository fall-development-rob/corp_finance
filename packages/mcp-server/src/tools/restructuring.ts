...
import { analyzeRecovery, analyzeDistressedDebt } from "corp-finance-bindings";
...
export function registerRestructuringTools(server: McpServer) {
  server.tool(
    ...
    async (params) => {
      const validated = RecoverySchema.parse(params);
  ...
  server.tool(
    ...
    async (params) => {
      const validated = DistressedDebtSchema.parse(params);
