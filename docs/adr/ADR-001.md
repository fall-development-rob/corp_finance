...
crates.io) provides 195 MCP tools spanning 67 domain modules and 67 CLI subcommands for
...
All 195 MCP tools remain the exclusive computational layer. Agents NEVER perform financial
...
- **Institutional-quality output**: Specialist agents with domain-specific tool access produce
  deeper analysis than a generalist single agent attempting to use all 195 tools
...
- **Extensibility**: Adding new specialist agents (e.g., a Structured Products Analyst) requires
  only defining which subset of the 195 tools it accesses and what its reasoning loop looks like
...
### A. Enhanced Single Agent with All 195 Tools
...
expertise across 67 domains. Tool selection degrades as tool count grows. No path to parallel
execution.
