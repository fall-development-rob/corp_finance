...
The corp-finance-mcp server exposes **195 tools** across 67 modules. The static `AGENT_TO_MCP`
map contained only **76 entries (39% coverage)**. This caused three compounding failures:

1. **119 MCP tools had no resolver mapping.** Agents constructing prefixed names for these
...
### Tier 2: Extended Static Map (76 -> 195 Entries)
...
- **100% tool coverage.** All 195 corp-finance-mcp tools reachable by at least one specialist.
...
- **No runtime performance impact.** Tiers 1-2 are O(1) lookups. Tier 3 is O(P) with ~60
  prefixes, activated only on miss — and rarely after expanding to 195 entries.
...
Rename all 195 MCP tools to use domain-prefixed names, eliminating the resolver.
