...
**Rejected as primary source because**: Alpha Vantage covers approximately 5,000 symbols compared to FMP's
...
**Update (ADR-014)**: Alpha Vantage was subsequently added as a *supplementary* data source
(43 tools) in `packages/data-mcp-server/src/alphavantage/`. It fills gaps that FMP does not
cover: AI news sentiment, server-side technical indicators (50+), commodity spot prices, and a
comprehensive single-call company overview. It is not a replacement for FMP but a complement.
